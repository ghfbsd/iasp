     ...
     1 ndex(ndp1,2),n
      ...
      data nout,delx,dpmax,drmax,tol,dtol,xtol/1,2*200.,2*.01,
     1 2*75.,1e-6,1d-6,1./
c
c     call ieeeset('environment')
      open(10,file='remodl1.lis')
      call rough(50.,nk,ncr,dmax,modnam)
      a0=r(1)
      write(10,200)nk,ncr,a0,dmax,(i,r(i),vp(i),vs(i),i=1,nk)
 200  format(/1x,2i5,2f10.2/(i5,f10.2,2f10.4))
      pn=vs(1)
      xn=1/r(1)
      tn=pn*xn
      do 1 i=1,nk
...
c   Sanity check to prevent faulty tables from being built below

      do i=1,mt(1)
         if (zm(i,1).lt.zmax) exit
      enddo
      plim = pm(min(i,mt(1)),1)
      do i=1,mt(2)
         if (pm(i,2).lt.plim) exit
      enddo
      if (zm(min(i,mt(2)),2).lt.zoc) then
         call die('**REMODL:  S slowness for max source depth is' //
     &            'in core; make max depth shallower.')
      endif
c
c   Loop over phases.
c
...
c
      subroutine rough(dr,n,ncr,dmax,modnam)
c
...
c
 ...
 7    if(u(i,1).le.u(j,1)) go to 2
      ifl=2
      call crtpt(j,1,'top low vel./high slow. zone P')
      go to 2
 8    if(u(i,1).ge.u(j,1)) go to 2
      ifl=1
      call crtpt(j,1,'bot. low vel./high slow. zone P')
 2    if(i.gt.ncr) go to 5
      go to (3,4),kfl
 3    if(u(i,2).le.u(j,2)) go to 5
      kfl=2
      call crtpt(j,2,'top low vel./high slow. zone S')
      go to 5
 4    if(u(i,2).ge.u(j,2)) go to 5
      kfl=1
      call crtpt(j,2,'bot. low vel./high slow. zone S')
 5    j=i
      ucrt(nc+1)=0.
      return
      end
...
c
      ...
      data nc/0/,tol/1e-6/
      if (nc.eq.0) then
         write(10,'(/a)') 'Critical point rationale:'
      else
c        Eliminate duplicates.
     ...
     &            u(k,nph),' (',why,')'
      ...
      nc=nc+1
      ucrt(nc)=u(k,nph)
      write(10,'(i2,2(1x,f10.6),1x,f7.1,2h: ,a)')
     &   nc,ucrt(nc),z(k),(1-exp(z(k)))/xn,why
...
c   No discontinuity.
      go to (29,30),isw
c   Not in a high slowness zone.  Have we hit a high slowness zone?
...
c   Eliminate the previous breakpoint and branch name if new one is at the same
c     breakpoint level (or higher, but not sure how this arises).  Replication
c     happens at the bottom of the severe LVZ in pem-c, for example.
...
c     routine has two purposes:
c        1) keep track of breakpoint succession so that a tau branch can be
c           named properly;
c        2) add a branch name to the phase table.
c     lc - number of named phases so far (updated)
c     nph - phase type: P = 1, S = 2
c     z0 - flattened earth depth
c     kfl - if > 0, add branch name to phase table, if <= 0 skip adding phase.

c     phase naming scheme as successive breakpoints encountered is (for eg P):
c     Pg, Pb, PmP, Pn, P, [PxxxP, P,] ... PcP, PKP, PKiKP, PKIKP
c     here xxx is a disc. depth and may be be repeated; if a PxxxP appears, a P
c     branch will appear after it.
...
c     This eliminates Pn & Sn for models without any mantle discontinuities in
c        favor of simply P & S.
      if(code(lc,nph)(2:4).eq.'SKS'.and.code(lc-1,nph)(2:3).eq.'Sn')
     1   code(lc-1,nph)(2:3)='S'
      if(code(lc,nph)(2:4).eq.'PKP'.and.code(lc-1,nph)(2:3).eq.'Pn')
     1   code(lc-1,nph)(2:3)='P'
